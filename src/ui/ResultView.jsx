...
export default function ResultView({
  ...
  useEffect(() => {
    const page = resultPageRef.current;
    const hero = heroRef.current;

    if (!page || !hero || !resultDeityImage) {
      return undefined;
    }
    ...
    const renderParallax = (scrollY) => {
      ...
      page.style.setProperty("--cloud-one-parallax-y", `${-3.5 * cloudOneProgress}rem`);
      page.style.setProperty("--cloud-two-parallax-x", `${-52 * cloudTwoProgress}vw`);
      page.style.setProperty("--cloud-two-parallax-y", `${-4.2 * cloudTwoProgress}rem`);
      page.style.setProperty("--ocean-parallax-x", `${-58 * oceanProgress}vw`);
  ...
  }, [resultDeityImage, selectedMatch?.deity_id]);
  ...
  return (
    ...
    >
      ...
      {resultDeityImage ? (
        <div
          aria-hidden="true"
          className="result-deity-parallax-overlay"
